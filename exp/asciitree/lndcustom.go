...
type BoxRenderOptions struct {
	Settings multistrings.BoxSettings
	Pattern  multistrings.BoxPattern
}
...
func GetLndCustom(ropt *LndRenderOptions) func(*tree.Node) ([]string, []string) {
	return func(n *tree.Node) ([]string, []string) {
		var label multistrings.MultiStrings
		var description multistrings.MultiStrings
...
func getLinesAndFormat(n *tree.Node, valueName string, formatLeft, formatRight string, mustache bool, extraLinesBefore int, extraLinesAfter int) multistrings.MultiStrings {
	if value, ok := n.Values()[valueName]; ok {
		lns := multistrings.MultiStrings(strings.Split(value.(string), "\n"))
	...
	return multistrings.New(0)
}

func getAttributes(n *tree.Node, prefix string) (maxWidth int, cut bool, box multistrings.BoxStyle, accolades multistrings.AccoladesStyle) {
	...
	if b, ok := n.Values()[prefix+"Box"]; ok {
		box, _ = multistrings.BoxStyleFromString(b.(string))
	} else {
		box = multistrings.BoxStyleNone
	}

	if a, ok := n.Values()[prefix+"Accolades"]; ok {
		accolades, _ = multistrings.AccoladesStyleFromString(a.(string))
	} else {
		accolades = multistrings.AccoladesStyleNone
	}
...
func formatLines(linesIn multistrings.MultiStrings, maxWidth int, cut bool, box multistrings.BoxStyle, accolades multistrings.AccoladesStyle, extraLinesBefore int, extraLinesAfter int) multistrings.MultiStrings {
	if maxWidth > 0 {
		if cut {
	...
	if box != multistrings.BoxStyleNone {
		linesIn = linesIn.AutoBox(multistrings.BoxSettings{
			Width:            maxWidth,
			TopLabel:         "",
			TopLabelAlign:    multistrings.LabelAlignLeft,
			BottomLabel:      "",
			BottomLabelAlign: multistrings.LabelAlignLeft}, multistrings.GetBoxPattern(box))
	}

	if accolades != multistrings.AccoladesStyleNone {
		linesIn = linesIn.AutoAccolades(accolades)
	}

	if extraLinesBefore > 0 {
		linesIn = multistrings.NewWithPresetContent("", extraLinesBefore).Append(linesIn)
	}

	if extraLinesAfter > 0 {
		linesIn = linesIn.Append(multistrings.NewWithPresetContent("", extraLinesAfter))
	}

	return linesIn
}

func colorizeLinesHTML(linesIn multistrings.MultiStrings, hexColor string) multistrings.MultiStrings {
...
func colorizeLinesConsole(linesIn multistrings.MultiStrings, hexColor string) multistrings.MultiStrings {
